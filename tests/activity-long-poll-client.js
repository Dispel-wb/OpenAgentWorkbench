...
async function main() {
  ...
  const queueActivity = await queueWait;
  ...
  assert(Array.isArray(queueActivity.data.queue) && queueActivity.data.queue.some(item => item.id === queued.id), 'Queue snapshot does not contain the enqueued item');
  ...
  const approvalActivity = await approvalWait;
  assert(approvalActivity.elapsedMs < 3000, `Approval creation did not wake the activity channel (${approvalActivity.elapsedMs}ms)`);
  assert(approvalActivity.data.approvals.some(item => item.id === approval.id), 'Approval snapshot is missing the pending request');
  ...
  const responseActivity = await responseWait;
  assert(responseActivity.elapsedMs < 3000, `Approval response did not wake the activity channel (${responseActivity.elapsedMs}ms)`);
  assert(!responseActivity.data.approvals.some(item => item.id === approval.id), 'Resolved approval remained in the pending snapshot');
