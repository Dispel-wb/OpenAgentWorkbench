...
$env:CLAUDE_GUI_WORKSPACE=$root;$env:CLAUDE_GUI_ROOT='D:\softwares\ClaudeCode'
...
try{
 ...
 [IO.File]::WriteAllText((Join-Path $dataRoot 'native-crash.log'),'Authorization: Bearer sk-diagnostic-secret-123456789',[Text.UTF8Encoding]::new($false))
 ...
 if($joined.Contains('sk-diagnostic-secret-123456789')){throw 'Diagnostic bundle leaked API key'}
...
}finally{if($hostProcess-and-not$hostProcess.HasExited){Stop-Process -Id $hostProcess.Id -Force -ErrorAction SilentlyContinue}}
