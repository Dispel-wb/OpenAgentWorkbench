...
function Wait-Until([scriptblock]$Action, [scriptblock]$Predicate, [int]$Seconds = 25) {
    ...
    throw 'Timed out waiting for MCP visual Host'
}

$root = Join-Path ([IO.Path]::GetTempPath()) ('claude-mcp-ui-' + [guid]::NewGuid().ToString('N'))
$root = [IO.Path]::GetFullPath($root)
$tempRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath())
if (-not $root.StartsWith($tempRoot, [StringComparison]::OrdinalIgnoreCase)) { throw 'Unsafe MCP visual fixture root' }
[IO.Directory]::CreateDirectory($root) | Out-Null
[IO.File]::WriteAllText((Join-Path $root '.mcp.json'), '{"mcpServers":{"fixture-runtime":{"command":"fixture-mcp.exe","args":["--stdio"],"env":{"FIXTURE_SECRET":"fixture-env-secret"}}}}', [Text.UTF8Encoding]::new($false))
...
$env:CLAUDE_GUI_MUTEX_SCOPE = 'mcp-ui-' + [guid]::NewGuid().ToString('N')
$hostProcess = $null
try {
    ...
    & $Node (Join-Path $PSScriptRoot 'mcp-runtime-ui-visual.js')
    if ($LASTEXITCODE -ne 0) { throw "MCP visual test failed with exit code $LASTEXITCODE" }
} finally {
