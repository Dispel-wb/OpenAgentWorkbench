...
try {
    ...
    if ($null -eq $runtime -or $runtime.state -ne 'running') { throw 'Long-session Host did not start' }
    ...
    & $Node (Join-Path $PSScriptRoot 'long-session-ui-visual.js')
    if ($LASTEXITCODE -ne 0) { throw 'Long-session UI visual test failed' }
}
finally {
