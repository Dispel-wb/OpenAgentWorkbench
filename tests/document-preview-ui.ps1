...
try {
    ...
    if ($null -eq $runtime -or $runtime.state -ne 'running') { throw 'Document-preview Host did not start' }
    ...
    & $Node (Join-Path $PSScriptRoot 'document-preview-ui.js')
    if ($LASTEXITCODE -ne 0) { throw 'Document-preview UI visual test failed' }
}
finally {
