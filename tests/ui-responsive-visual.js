...
async function main() {
  ...
  try {
    ...
    if(!Object.entries(coldFileControls).filter(([key])=>!['edition','skin'].includes(key)).every(([,value])=>isBlue(value)))throw new Error('Developer workbench or peripheral controls retain a warm color: '+JSON.stringify(coldFileControls));
    ...
    const blue=values=>values[2]>values[0]&&values[2]>values[1],brown=values=>values[0]>values[2]&&values[1]>values[2],brightness=values=>values.reduce((sum,value)=>sum+value,0)/values.length;
    if(!blue(settingsOptionColors.fusionDark)||brightness(settingsOptionColors.fusionDark)>=100||!blue(settingsOptionColors.fusionLight)||brightness(settingsOptionColors.fusionLight)<=180||(settingsOptionColors.edition==='local'?!brown(settingsOptionColors.claude):!blue(settingsOptionColors.claude)))throw new Error('Settings option theme colors violate the dark-navy/light-blue and local-Claude brown contract: '+JSON.stringify(settingsOptionColors));
    ...
    const apiControlColors=await page.evaluate(()=>{const color=(selector,property)=>{const value=getComputedStyle(document.querySelector(selector))[property];return(value.match(/\d+(?:\.\d+)?/g)||[]).slice(0,3).map(Number);};const read=()=>({inputBackground:color('.api-settings-pane input','backgroundColor'),inputBorder:color('.api-settings-pane input','borderTopColor'),groupBackground:color('.provider-autofill optgroup','backgroundColor'),groupText:color('.provider-autofill optgroup','color'),iconBackground:color('.cap-icon','backgroundColor'),iconBorder:color('.cap-icon','borderTopColor'),iconText:color('.cap-icon','color'),heading:color('.capability-head h3','color')});const edition=document.documentElement.dataset.edition;document.documentElement.dataset.skin='fusion';document.documentElement.dataset.theme='dark';const fusionDark=read();document.documentElement.dataset.theme='light';const fusionLight=read();document.documentElement.dataset.skin='claude';document.documentElement.dataset.theme='dark';const claude=read();document.documentElement.dataset.skin='fusion';return{edition,fusionDark,fusionLight,claude};});
    ...
    if(!allCold(apiControlColors.fusionDark)||brightness(apiControlColors.fusionDark.inputBackground)>=100||!allCold(apiControlColors.fusionLight)||brightness(apiControlColors.fusionLight.inputBackground)<=180||(apiControlColors.edition==='local'?!allWarm(apiControlColors.claude):!allCold(apiControlColors.claude)))throw new Error('API controls violate dark-navy/light-blue, select-group or local-Claude color rules: '+JSON.stringify(apiControlColors));
    await page.screenshot({path:path.join(outputDir,'ui-api-control-colors.png')});
    await page.locator('.settings-nav button').filter({hasText:'外观与 Agent'}).click();
    const appearanceControls=await page.evaluate(()=>{const rgb=()=>getComputedStyle(document.querySelector('.appearance-heading .secondary')).backgroundColor.match(/\d+(?:\.\d+)?/g).slice(0,3).map(Number);document.documentElement.dataset.theme='dark';const dark=rgb();document.documentElement.dataset.theme='light';const light=rgb();document.documentElement.dataset.theme='dark';return{dark,light};});
    if(!blue(appearanceControls.dark)||brightness(appearanceControls.dark)>=100||!blue(appearanceControls.light)||brightness(appearanceControls.light)<=180)throw new Error('Appearance secondary controls are not dark navy and light blue by theme: '+JSON.stringify(appearanceControls));
    await page.evaluate(()=>document.documentElement.dataset.theme='light');
    await page.screenshot({path:path.join(outputDir,'ui-appearance-controls-light.png')});
    await page.evaluate(()=>document.documentElement.dataset.theme='dark');
    await page.locator('.settings-nav button').filter({hasText:'已归档聊天'}).click();
    const archiveColors=await page.evaluate(()=>{const color=(selector,property)=>{const value=getComputedStyle(document.querySelector(selector))[property];return(value.match(/\d+(?:\.\d+)?/g)||[]).slice(0,3).map(Number);};const read=()=>({browser:color('.archive-browser','backgroundColor'),border:color('.archive-browser','borderTopColor'),empty:color('.archive-empty','color'),symbol:color('.archive-preview-empty span','color'),clear:color('.archive-clear','backgroundColor'),clearText:color('.archive-clear','color')});document.documentElement.dataset.theme='dark';const dark=read();document.documentElement.dataset.theme='light';const light=read();document.documentElement.dataset.theme='dark';return{dark,light};});
    if(!Object.values(archiveColors.dark).every(blue)||brightness(archiveColors.dark.browser)>=100||!Object.values(archiveColors.light).every(blue)||brightness(archiveColors.light.browser)<=180)throw new Error('Empty archive surfaces retain warm or incorrectly dark colors: '+JSON.stringify(archiveColors));
    await page.screenshot({path:path.join(outputDir,'ui-empty-archive-colors.png')});
    await page.evaluate(()=>document.documentElement.dataset.theme='light');
    await page.screenshot({path:path.join(outputDir,'ui-empty-archive-light.png')});
    await page.evaluate(()=>document.documentElement.dataset.theme='dark');
    ...
    process.stdout.write(JSON.stringify({ responsiveUi: 'PASS', results, lightTheme, reducedMotion, coldFileControls, runCenter, approvalLayout, memoryLayout, memoryEditor, memoryLight, runtimeLimits, startupLayout, settingsOptionColors, apiControlColors, appearanceControls, archiveColors, startupRegistration:'PASS', dialogAccessibility:{settings:true,focusTrap:true,focusReturn:true,palette:true}, rapidSessionSwitch:'PASS',messageContextMenu:{deleteOnly,selectionActions},activitySync:{idle:idlePolling,fallback:activePolling} }, null, 2));
