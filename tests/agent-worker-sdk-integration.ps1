...
try {
    ...
    $output = $process.StandardOutput.ReadToEnd(); $bridgeError = $process.StandardError.ReadToEnd()
    ...
    foreach ($failureCase in @('fixture-terminal-failure','fixture-no-terminal')) {
        ...
        try {
            ...
            if ($failureResults.Count -ne 1 -or -not $failureResults[0].is_error) { throw "Codex failure was reported as success: $failureCase" }
    ...
    if ($null -eq $runtime -or $runtime.state -ne 'running') { throw 'Providerless Host did not start.' }
    ...
    $lostRunId=[guid]::NewGuid().ToString();$lostRoot=Join-Path $root ".claude-gui-v2\runs\$lostRunId"
    [IO.Directory]::CreateDirectory($lostRoot)|Out-Null
    $stored.workerHarness='dsh';$stored.guiSessionId=$lostRunId;$stored.sessionId=$lostRunId
    ...
    if($lostStatus.state -ne 'failed' -or $lostStatus.message -notmatch '未自动重放' -or (Test-Path (Join-Path $lostRoot 'pid.txt'))){throw 'Lost DSH core was automatically replayed without resume support'}
    [pscustomobject]@{ AgentWorkerSdk = 'PASS'; Protocol = 'claude-stream-json-v1'; CodexTurns = $results.Count; Utf8 = $true; ResumeState = $true; ProviderlessHostRun = $true; TerminalFailurePreserved = $true; UnsupportedApprovalDenied = $true; ActiveMemoryDelivered = $true; LostDshReplaySuppressed = $true } | Format-List
} finally {
