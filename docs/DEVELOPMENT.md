...
这套 69 项回归是离线验证，不替代原生键盘/读屏器、干净 Windows 矩阵、付费模型服务、独立安全复核或 72 小时稳定性验收。子进程不继承 Anthropic 令牌与地址形成的默认 Provider；各测试必须显式准备自己的离线配置。100 次故障恢复仍全部执行，但该项在较慢 CI 主机上单独允许至少 900 秒，其余项使用所配置的超时。
