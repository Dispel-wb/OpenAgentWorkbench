...
- Unified settings, peripheral controls and the complete developer workbench chrome on deep blue for Open Agent, Codex and mixed skins in both light and dark modes, including top selectors, panels, dividers, file-tree glyphs, API capability cards, text and matched inner/outer focus frames; only the Local Claude skin retains the warm-brown treatment.
